...
onMounted(async () => {
  ...
  try {
    // 회원 정보 조회(쿠키/세션 기반)
    const res = await axios.get('http://13.209.155.21:8080/users/me', {
    ...
    // 회원 채팅방 목록 조회
    const roomsResponse = await axios.get('http://13.209.155.21:8080/chatrooms', {
      withCredentials: true
    })
...
const handleLogout = async () => {
  await axios.post('http://13.209.155.21:8080/logout', null, {
    withCredentials: true
  })
  localStorage.removeItem('username')
  location.reload()
